 ...
 */
public class InterestTagRecall implements Recall {
    @Override
    public void train() {
    ...
    public void run() {
        try {
            ResultSet user_interestTag = mySqlJDBC.getInstance().read(
                    "SELECT id,interest_tags FROM `user`"
            );
            ...
            for (String u_id : id2tags.keySet()
                ...
                String sql = String.format("UPDATE user_rec SET interestTag_recall = \"%s\" WHERE user_id = %s;\n",  item_ids , u_id);
    ...
    public static void main(String[] args) {
        InterestTagRecall in = new InterestTagRecall();
        in.run();
    }
}
