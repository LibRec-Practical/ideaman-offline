...
import net.librec.recommender.cf.ranking.LDARecommender;
...
public class TopicRecall implements Recall {
    public TopicRecall() throws IOException, LibrecException {


    }

    //保存推荐结果
    public static void saveResult(List<RecommendedItem> recommendedList) throws LibrecException, IOException, ClassNotFoundException, TException {
        ...
        if (recommendedList != null && recommendedList.size() > 0) {
            ...
            for (String uid : uid2items.keySet()) {
                List<String> papers = uid2items.get(uid);
                String item_ids = String.join(",", papers.toArray(new String[0]));
                String sql = String.format("UPDATE user_rec SET topic_recall = \"%s\" WHERE user_id = %s;\n", item_ids, uid);
    ...
    public static void main(String[] args) throws IOException, LibrecException {
        TopicRecall tp = new TopicRecall();
        tp.run();
    }
}
